...
class SystemResultSet implements ResultSet {
    ...
    public BigDecimal getBigDecimal(int columnIndex, int scale) throws SQLException {
        Object value = getObject(columnIndex);
        return value == null ? null : (new BigDecimal(value.toString()).setScale(scale, BigDecimal.ROUND_HALF_UP));
    }
    ...
    public boolean relative(int rows) throws SQLException {
        this.row = (row + rows) % this.data.size();
        return row >= 0 && row < this.data.size();
    }
    ...
    public Object getObject(int columnIndex) throws SQLException {
        return data.get(row)[columnIndex - 1];
    }
